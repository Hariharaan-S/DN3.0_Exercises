...
public class CustomerController {
    ...
    @PostMapping(value = "/create/customer", consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<CustomerDTO> createANewCustomer(@RequestBody Customer newCustomer) {
        try {
            return new ResponseEntity<>(customerService.convertToCustomerDto(customerService.createNewCustomer(newCustomer)), HttpStatusCode.valueOf(201));
        } catch (Exception e) {
    ...
    @GetMapping(value = "/get/customers", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    public ResponseEntity<List<CustomerDTO>> getAllCustomers() {
        return new ResponseEntity<>(customerService.getAllCustomers(), HttpStatusCode.valueOf(200));
    }
    @GetMapping(value = "/get/customer/{id}", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
